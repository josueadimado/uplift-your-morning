...
class NotificationSendNowView(StaffRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        # Send SMS - FIXED Bug 1: Group errors by error message directly, not by error type
        sms_sent = 0
        sms_failed = 0
        sms_errors = {}
        if whatsapp_subscribers:
            for subscriber in whatsapp_subscribers:
                try:
                    ...
                    sms_failed += 1
                    error_msg = str(e)
                    # FIXED: Group by error message directly, prevents corruption when multiple error types are mixed
                    if error_msg not in sms_errors:
                        sms_errors[error_msg] = []
                    sms_errors[error_msg].append(subscriber.phone)
        
        # Display SMS errors - FIXED: Show grouped errors properly
        if sms_errors:
